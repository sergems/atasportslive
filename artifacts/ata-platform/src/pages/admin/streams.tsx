...
const EMPTY_FORM = {
  title: '', description: '', sport: 'pool', startTime: '', endTime: '',
    ...
    <Card className={`bg-slate-900 border ${accentClass}`}>
      ...
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-1">
          ...
          <Input value={form.title} onChange={(e: any) => setForm({ ...form, title: e.target.value })} className="bg-slate-800 border-slate-700 text-white" />
        </div>
        <div className="space-y-1">
        ...
        <div className="space-y-1">
          <Label className="text-slate-300">{isTournament ? 'Start Date & Time' : 'Date & Time'}</Label>
        ...
        <div className="space-y-1">
          <Label className="text-slate-300">{isTournament ? 'End Date & Time (range)' : 'End Time (optional)'}</Label>
        ...
        {!isTournament && onFileChange && (
          <p className="md:col-span-2 text-xs text-teal-400/80">A matching game will be automatically created for betting when this stream is saved.</p>
        )}

        <div className="md:col-span-2 flex gap-3">
          <Button onClick={onSave} disabled={saving} className="bg-teal-500 hover:bg-teal-400 text-slate-950">{saving ? 'Saving...' : 'Save'}</Button>
...
export default function AdminStreams() {
  ...
  const handleCreate = async () => {
    ...
    if (!isTournament && (!form.playerA || !form.playerB)) { toast.error('Player A and Player B required'); return; }
    try {
      setUploading(true);
      const thumbnailUrl = await uploadThumbnail();
      await createStream.mutateAsync({ data: { title: form.title, description: form.description, sport: form.sport, startTime: form.startTime, endTime: form.endTime || undefined, accessPrice: parseFloat(form.accessPrice), thumbnailUrl: thumbnailUrl ?? undefined, playerA: isTournament ? undefined : form.playerA, playerB: isTournament ? undefined : form.playerB, city: form.city || undefined, country: form.country || undefined } as any });
      invalidate();
      toast.success(`Stream created${isTournament ? '' : ' — a matching game was also created'}`);
      setShowForm(false);
      setForm(EMPTY_FORM);
      clearThumbnail();
    } catch (e: any) { toast.error(e?.message || e?.data?.error || 'Failed'); }
  ...
  const startEdit = (stream: any) => {
    ...
    setEditForm({
      ...
      sport: stream.sport || 'pool', startTime: toLocalDatetime(stream.startTime),
      endTime: toLocalDatetime(stream.endTime), accessPrice: String(stream.accessPrice ?? '1.50'),
      playerA: '', playerB: '', city: stream.city || '', country: stream.country || '',
    });
    setShowForm(false);
  };

  const handleEdit = async () => {
    if (!editForm.title || !editForm.startTime) { toast.error('Title and start time required'); return; }
    try {
      setEditSaving(true);
      await updateStream.mutateAsync({ id: editId!, data: { title: editForm.title, description: editForm.description, sport: editForm.sport, startTime: editForm.startTime, endTime: editForm.endTime || undefined, accessPrice: parseFloat(editForm.accessPrice), city: editForm.city || undefined, country: editForm.country || undefined } as any });
      invalidate();
  ...
  const handleGoLive = async (id: number) => {
    ...
    try {
      ...
      invalidate(); toast.success('Stream is now LIVE'); setGoLiveId(null); setHlsUrl('');
    } catch (e: any) { toast.error(e?.data?.error || 'Failed'); }
  };

  const handleEnd = async (id: number) => {
    try { await endStream.mutateAsync({ id }); invalidate(); toast.success('Stream ended'); }
    catch (e: any) { toast.error(e?.data?.error || 'Failed'); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setShowForm(!showForm); setEditId(null); }} className="bg-teal-500 hover:bg-teal-400 text-slate-950">
          <Plus className="h-4 w-4 mr-1" /> New Stream
        </Button>
      </div>

      {showForm && (
        <StreamForm form={form} setForm={setForm} isTournament={isTournament}
          thumbnailPreview={thumbnailPreview} fileInputRef={fileInputRef}
          onFileChange={handleFileChange} onClearThumb={clearThumbnail}
          onSave={handleCreate} onCancel={() => { setShowForm(false); clearThumbnail(); setForm(EMPTY_FORM); }}
          saving={createStream.isPending || uploading} title="Create Stream" accentClass="border-teal-500/30" />
      )}
        ...
        <div className="space-y-3">
          {(streamsData?.streams || []).map((stream: any) => (
            <div key={stream.id}>
                ...
                <CardContent className="py-4 px-5 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className="flex-shrink-0 w-14 h-10 rounded overflow-hidden bg-slate-800 flex items-center justify-center">
                      {stream.thumbnailUrl ? <img src={stream.thumbnailUrl} alt={stream.title} className="w-full h-full object-cover" /> : <ImageIcon className="h-5 w-5 text-slate-600" />}
                    </div>
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <Button size="sm" variant="ghost" onClick={() => editId === stream.id ? setEditId(null) : startEdit(stream)} className="h-8 w-8 p-0 text-slate-400 hover:text-amber-400">
              ...
              {editId === stream.id && (
                <div className="mt-2">
                  <StreamForm form={editForm} setForm={setEditForm} isTournament={editIsTournament}
